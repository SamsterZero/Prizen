...
export type TrackerWorkerOptions = {
	sql: Sql;
	appUrl: string;
	trackerToken: string;
...
export function createTrackerWorker(options: TrackerWorkerOptions) {
	const { sql, appUrl, trackerToken, logger } = options;
			...
			try {
				...
				const response =
					notification.provider === 'discord'
						? await fetcher(decryptSecret(notification.secret_reference), {
								method: 'POST',
						...
						: await (async () => {
								...
								return fetcher(`https://api.telegram.org/bot${destination.botToken}/sendMessage`, {
									method: 'POST',
									headers: { 'content-type': 'application/json' },
	...
	async function scanDueProducts() {
		...
		const due = await sql<DueProduct[]>`with due as (
			select job.id as job_id, product.id, product.url, product.polling_interval_seconds,
				job.attempts, product.target_price, product.user_id
			from scan_jobs job
		...
		returning due.job_id, due.id, due.url, due.polling_interval_seconds, due.attempts,
			due.target_price, due.user_id`;
		for (const product of due) {
			try {
				const response = await fetcher(`${appUrl}/api/products/preview`, {
					method: 'POST',
					headers: {
	...
	return { scanDueProducts, deliverPendingNotifications, recordHeartbeat };
}

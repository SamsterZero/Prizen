...
apply_stable_migrations() {
	...
	while read -r _ migration; do
		"${compose[@]}" exec -T db psql -U prizen -d prizen -v ON_ERROR_STOP=1 \
...
apply_stable_migrations
"${compose[@]}" exec -T db psql -U prizen -d prizen -v ON_ERROR_STOP=1 -c \
...
marker_count="$("${compose[@]}" exec -T db psql -U prizen -d prizen -At -c \
	"SELECT count(*) FROM marketplaces WHERE slug = '$marker_slug';")"
migration_count="$("${compose[@]}" exec -T db psql -U prizen -d prizen -At -c \
